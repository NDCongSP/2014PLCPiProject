...
def beginAsynchronousTest(client):
    ...
    dassert(rr, lambda r: r.bits == [True]*8)         # test the expected value
    '''
    rq = client.write_register(1, 115)
    rr = client.read_holding_registers(4,1)
    #dassert(rq, lambda r: r.function_code < 0x80)     # test that we are not an error
    #dassert(rr, lambda r: r.registers[0] == 1111)       # test the expected value
...
#---------------------------------------------------------------------------# 
defer = protocol.ClientCreator(reactor, ModbusClientProtocol).connectTCP('192.168.1.10', Defaults.Port)
